...
export default function ProfessionalTemplate() {
  ...
  const ensureArray = useWork((state: any) => state.ensureArray);
  ...
  // Ensure companies array is properly initialized
  useEffect(() => {
    if (ensureArray) {
      ensureArray();
    }
  ...
  let leftSections = [
    {
      title: labels[0],
      icon: labelsIcon[0],
      component: <Exp companies={safeCompanies} />,
      styles: { flexGrow: 1 },
    },
    {
      title: labels[2],
  ...
  return (
    <ResumeContainer>
      <LeftSection>
        <ModernHeaderIntro displaySocial={false} title={intro.name} profiles={intro.profiles} verification={verification}>
