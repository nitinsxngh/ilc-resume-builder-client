  ...
  persist(
    (set, get) => ({
      ...
      reset: (data = userData.work) => {
        set({ companies: Array.isArray(data) ? data : [] });
      },
      ...
      purge: (index: number) =>
        set((state: any) => {
          const currentCompanies = state.companies || [];
          return {
            companies: currentCompanies.filter((_, ind) => ind !== index),
          };
        }),
      ...
      changeOrder: ({ oldIndex, newIndex }) =>
        set((state: any) => {
          const currentCompanies = state.companies || [];
          return {
            companies: arrayMoveImmutable(currentCompanies, oldIndex, newIndex),
          };
        }),
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        // Ensure companies is an array after rehydration
